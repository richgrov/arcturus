...
export const createSongRecord = functions.storage.onObjectFinalized(
  async (event) => {
    ...
    const file = getStorage().bucket(event.data.bucket).file(filePath);

    const metadata = await file
      .download()
      .then((data) => musicMetadata.parseBuffer(data[0]));
